package com.lucas.example.prime;

import com.lucas.example.prime.util.PrimeNumberUtilTest;
...
public class PrimeControllerTest {
    ...
    public void testPrimeGeneration() {
        ...
        ResponseEntity<int[]> entity = restTemplate.getForEntity(baseUrl + "/primes", int[].class);
        int[] expected = {2, 3, 5, 7, 11, 13, 17, 19};
        checkBody(entity, expected);

        // Test negative upper bound
        entity = restTemplate.getForEntity(baseUrl + "/primes?upperBound=-7919", int[].class);
        expected = new int[]{};
        checkBody(entity, expected);
    ...
    /**
     * Test the /primes/{number} API resource using combinations of prime and composite numbers and 3 different algorithms
     */
    @Test
